...
public class SuggectionMenu extends AppCompatActivity implements View.OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_suggection_menu);
        StrictMode.ThreadPolicy policy = new StrictMode.ThreadPolicy.Builder().permitAll().build();
        StrictMode.setThreadPolicy(policy);

        objTempStorage=new TempStorage(getApplicationContext());

        etdiseas = (EditText) findViewById(R.id.etdiseas);
        etsymptomps=(EditText) findViewById(R.id.etsymptomps);
    ...
    public void getData()
    {
        diseas=etdiseas.getText().toString().trim();

        symptomps=etsymptomps.getText().toString().trim();
        ...
        params1.add(new BasicNameValuePair("diseas", diseas));
        params1.add(new BasicNameValuePair("symptomps", symptomps));
        params1.add(new BasicNameValuePair("region", Region));


        json = HttpCommunication.makeHttpRequest(Constant.TEMP_SUGGECTION_UPDATE, "GET", params1,getApplicationContext());
        try {
